...
log "tldr pages (MIT)"
if [[ ! -d "$VENDOR/tldr/.git" ]]; then git clone --depth=1 https://github.com/tldr-pages/tldr.git "$VENDOR/tldr" >/dev/null; else (cd "$VENDOR/tldr" && git pull --ff-only >/dev/null); fi
copy_tree "$VENDOR/tldr/pages" "$DST/tldr"
